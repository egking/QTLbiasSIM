...
{
line.set<-sample(colnames(all.phenos[,4:ncol(all.phenos)]),samp.size)
ccc<-all.phenos[i,'CHROM']
ppp<-all.phenos[i,'POS']
...
qpos<-all.phenos[i,'POS']

if(length(which(set.p[,2]==qpos))<=1)
{
  set$QLOD[counter]<-mod[which(set.p[,2]==qpos),1]
}else{
  set$QLOD[counter]<-mod[which(rownames(set.p)==rownames(all.phenos)[i]),1]
}
...
fname<-paste("Data/DGRP/DGRPscansLOCO_",samp.size,"_",st.ind,"_",en.ind,".rda",sep="")
save(output, file=fname)

fname2<-paste("Data/DGRP/DGRPsetLOCO_",samp.size,"_",st.ind,"_",en.ind,".rda",sep="")
